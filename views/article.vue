<template>
	...
	<h1>	{{post.title}} </h1>
	<v-row>
		<v-col><div class="">{{ `${post.post_date}` | moment("from", "now") }}</div></v-col>
		<v-col><share  class="text-left" :link=this.$route.path  :ptitle=post.title :pdesc=post.body.substring(0,100) :tags=post.tags> </share></v-col>
		
		
	</v-row>
		
		<v-img v-if="gallery.length <= 1" :src="`${uploads}${post.original_image}`" width="100%" class="my-5"> </v-img>		
...
export default  {
	...
	async asyncData({ $axios,params }) {
		...
		return { post: data[0],gallery:JSON.parse(data[0].gallery_images) }			
},data(){
	return{
