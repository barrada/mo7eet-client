      ...
      <v-row  class="">
        ...
        <v-col id="restfeat_h" lg="6" md="6" sm="6" cols="12"  class="pl-8 hidden-xs-only">
                <v-row  class=""  v-for="(feat,$index) in restfeat" :class="{'rest_feat_border': $index < 3}" style="">              
										<v-card :id="`post-${feat.id}`" cols="12" class="" :to="{name:'recipe_article',params:{slug:feat.slug}}">
				...
				<v-col id="restfeat_vcard" xs="12" lg="6" md="6" sm="6" cols="12"  class="pt-0 mb-n4 hidden-sm-and-up">
					...
					>
						<v-card				
   						...
   						max-width="400"
							 	:id="`post-${feat.id}`"
								:to="{name:'recipe_article',params:{slug:feat.slug}}"				 					 
			  		>
							<v-img
								class=" white--text"
								height="200px"
  ...
  > 
			...
			<v-col v-for="(recipe,$index) in list" class="pa-1 col-md-3 col-xs-1 col-lg-3 col-sm-4">
				...
				<v-card
   				...
   				max-width="400"
					:id="`post-${recipe.id}`"
					:to="{name:'recipe_article',params:{slug:recipe.slug}}"
				>
					<v-img
						class="white--text"
						height="200px"
...
export default{
	...
	async asyncData({ $axios,query }) {
			...
			const data= await $axios.$get('https://mo7eet-server.herokuapp.com/api/posts/recipes')
...
methods:{
	...
	infiniteHandler($state){
		this.$axios.$get('https://mo7eet-server.herokuapp.com/api/posts/recipes',{params:{page:this.page}}).then((response=>{
