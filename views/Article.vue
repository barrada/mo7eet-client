<template>
  ...
  <!-- featured image and gallery -->
    ...
    <div  id="article" class="mt-5" v-html="post.body">
          {{post.body}}
    </div>
        </v-col>
...
export default {
  ...
  async asyncData({ $axios, params }) {
    ...
    return { post: data[0], gallery: gallery,title:data[0].title,bodySEO:data[0].body,tags:data[0].tags,excert:data[0].excert.substring(0,100) + '...',descritpion:data[0].body.substring(0,100) + '...',thumbnail:data[0].thumbnail,url:url,date:data[0].post_date,keywords:data[0].keywords };
  },
  data() {
    return {     
